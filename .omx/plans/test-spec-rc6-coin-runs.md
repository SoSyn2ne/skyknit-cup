...
- 탐험 모드에서는 현재 수집 가능한 동전 하나만 보이고, 수집 직후 다음 동전으로 표시 대상이 바뀐다.
- 현재 동전이 화면 밖에 있으면 44px 방향 표식이 viewport 안에 유지되고, 화면 안에서 충분히 크게 보이면 표식을 숨긴다.
- 지역 로드 범위가 겹쳐도 현재 지역 목표 하나만 보이며, 카메라 뒤쪽 목표의 좌우 방향이 반전되지 않는다.
- 일시정지, 지도, 착륙, 착륙 완료, 이륙 전환 중에는 동전 메시와 방향 표식을 모두 숨긴다.
- 축제 허브의 비수집 랜턴은 공유 금색 재질을 복제해 붉은 팔레트로 표시하고, 다른 금색 구조물 재질은 바꾸지 않는다.
