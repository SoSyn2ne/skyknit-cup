...
- 각 character ID는 고유 GLB URL과 pose profile을 가진다.
...
- 각 캐릭터는 18,000~20,500 triangles, 14 meshes, 18 primitives, 3 materials, gzip-9 450KiB 이하이다.
- 세 실루엣은 검은 mask 기준 bounding/profile 차이가 있고 그리핀은 부리·깃털, 가오리는 납작한 몸·지느러미·채찍 꼬리를 가진다.
- 고글·리본은 각각 3,000 triangles, 2 primitives, gzip-9 100KiB 이하이며 유한 transform/normal을 가진다.
- 같은 입력을 두 번 export한 SHA-256이 동일하다.

## RED 4 — CharacterWorkshop DOM

- ready와 paused에서 `캐릭터 꾸미기` 버튼을 제공하고 다른 단계에서는 숨긴다.
- dialog accessible name은 `캐릭터 공방`, 세 native select label은 `캐릭터 형태`, `색상`, `장식`이다.
...
2. 그리핀·달빛·고글 초안이 debug snapshot과 캔버스 픽셀을 바꾸되 localStorage는 바꾸지 않는지 확인한다.
3. 돌아가기와 Escape가 드래곤 기본 외형을 복구하는지 확인한다.
...
7. console/page/unhandled/network 오류가 0이고 nonblank/시간 변화 픽셀이 유지되는지 확인한다.
...
- `npm run test:production`: audit/QA hook/전송 예산 통과
- 캐릭터별 30초 high/low 성능: desktop 55/50fps, mobile 30fps, high 120 draw calls 이내
- 전체 gzip-9 10MiB 이하
- 다섯 screenshot 독립 `visual-verdict >= 90`
