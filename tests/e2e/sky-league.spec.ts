...
test.describe('Sky League offline record competition', () => {
  ...
  test('saves the first race and successful mission to v10 Top 10 boards with canonical ghosts', async ({
    page,
  }) => {
  ...
  test('saves a completed regional coin run to its v10 Top 10 board and ghost slot', async ({
    page,
  }) => {
    await page.goto('/')
    await expect(page.locator('#app')).toHaveAttribute(
...
test.describe('Sky League v7 migration', () => {
  test('migrates records and Festival discoveries to v10 without fabrication', async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== 'desktop',
